...
int main() {
	...
	auto char1 = CollidableEntity::create<Character>(spr1, col1);
	...
	auto char2 = CollidableEntity::create<Character>(spr2, col2);
	...
	process.run([&](float deltaTime) {
		...
		//renderer.drawColoredEntity(col1 , window.projection, { 1.0f, 0.0f, 0.0f, .5f });
	});
